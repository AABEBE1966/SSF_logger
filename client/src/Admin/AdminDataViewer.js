...
function AdminDataViewer(props) {
    ...
    useEffect(async () => {
        if(loading) {
            ...
            console.log("after the api caller");
...
const data = [
  ...
  { name: "Page F", uv: 220, pv: 2400, amt: 2400 },
...
function AdminDataViewer(props) {
  ...
  const generateAllMetricsData = () => {
    let ans = [];
    if (allMetricsData) {
      ans = Object.keys(allMetricsData).map((count_name, idx) => {
        return {
          name: count_name,
          uv: allMetricsData[count_name],
          pv: 2400,
          amt: 2400,
        };
      });
>>>>>>> 6c40fa4a9f2c26e74fcde41667c4fe9bb31a497e
    }
    setAllMetricsDataForGraph(ans);
  };
...
=======
  ...
  return (
    <div>
      ...
      <BarChart width={600} height={300} data={allMetricsDataForGraph}>
        <XAxis dataKey="name" stroke="#8884d8" />
        <YAxis />
        <Tooltip />
        <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
