...
router.post("/sign_up", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: "Admin account created successfully!" });
  } catch (err) {
    return res
      .status(201)
...
router.post("/sign_in", async function (req, res) {
  ...
  try {
    ...
    if (!admin) {
      ...
      return res.status(201).send({
        message: `Failed to log in. Please check your credentials and try again. Admin does not exist`,
      });
    }
    ...
    await admin.isCorrectPassword(password, async (err, same) => {
      if (err || !same) {
        ...
        await admin.save();
        let adminRes = await Admin.findById(admin._id, {
          password: 0,
          tokens: 0,
          email: 0,
        });
...
router.get("/log_out", withAuthAdmin, async function (req, res) {
  ...
  if (already_login && already_login["tokens"].indexOf(token) !== -1) {
    ...
    res.cookie("admin_token", "");
    res.cookie("isAuth", false);
    return res.status(201).send({ message: "Sucessfully Logged Out!" });
  }
});
...
router.get("/find_all_persons", withAuthAdmin, async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .send({ message: "Something is wrong. Please retry. " + err.message });
  }
});
...
router.get("/get_logged_in_admin", withAuthAdmin, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ admin: admin, message: "Logger data returned successfully!" });
  } catch (err) {
    return res
      .status(201)
      .send({ message: "Something is wrong. Please retry. " + err.message });
...
router.post("/find_all_persons_by_wereda/", withAuthAdmin, async (req, res) => {
...
router.get("/find_all_metrics", withAuthAdmin, async (req, res) => {
...
router.post(
  "/find_all_metrics_for_zone_with_wereda",
  withAuthAdmin,
  async (req, res) => {
    const { zone } = req.body;
    ...
    for (let i = 0; i < zone_wereda_arm_type_data.length; i++) {
      ...
      if (wereda_name in temp_data) {
        temp_data[wereda_name][arm_type] =
          temp_data[wereda_name][arm_type] + total_person;
        temp_data[wereda_name]["person"] =
          temp_data[wereda_name]["person"] + total_person;
        temp_data[wereda_name]["bullet"] =
...
router.get(
  "/find_all_metrics_for_region_with_zone",
  withAuthAdmin,
  async (req, res) => {
    ...
    for (let i = 0; i < region_zone_arm_type_data.length; i++) {
      ...
      if (zone_name in temp_data) {
        temp_data[zone_name][arm_type] =
          temp_data[zone_name][arm_type] + total_person;
        temp_data[zone_name]["person"] =
          temp_data[zone_name]["person"] + total_person;
        temp_data[zone_name]["bullet"] =
...
router.post("/find_all_metrics_for_zone", withAuthAdmin, async (req, res) => {
  const { zone } = req.body;
  console.log(zone);
...
router.post("/find_all_metrics_for_wereda", withAuthAdmin, async (req, res) => {
  ...
  const AK47 = await Person.find({
    zone: zone,
    wereda: wereda,
    armType: "AK 47",
  }).count();
  const brail = await Person.find({
    zone: zone,
    wereda: wereda,
    armType: "Brail",
  }).count();
  const dishka = await Person.find({
    zone: zone,
    wereda: wereda,
    armType: "Dishka",
  }).count();
  const abraraw = await Person.find({
    zone: zone,
    wereda: wereda,
    armType: "Abraraw",
  }).count();
  const guande = await Person.find({
    zone: zone,
    wereda: wereda,
    armType: "Guande",
  }).count();
