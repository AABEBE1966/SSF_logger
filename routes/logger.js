...
router.post("/sign_up", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: "Logger account created successfully!" });
  } catch (err) {
    return res
      .status(201)
  ...
  try {
    ...
    await logger.isCorrectPassword(password, async (err, same) => {
      if (err || !same) {
        ...
        await logger.save();
        let loggerRes = await Logger.findById(logger._id, {
          password: 0,
          tokens: 0,
          email: 0,
        });
...
router.post("/log_user_data", withAuthLogger, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: "Information uploaded successfully!" });
  } catch (err) {
    return res
      .status(201)
      .send({ message: "Failed to log user information. " + err.message });
  }
});
...
router.get("/get_logged_in_logger", withAuthLogger, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ logger: logger, message: "Logger data returned successfully!" });
  } catch (err) {
    return res
      .status(201)
      .send({ message: "Something is wrong. Please retry. " + err.message });
